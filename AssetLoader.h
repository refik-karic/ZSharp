...
class AssetLoader final {
  ...
  static void LoadModelJSON(const char* fileName, Model& model) {
    ...
    for (size_t meshIndex = 0; meshIndex < jsonObject.GetValue().dataArray.size(); ++meshIndex) {
      ...
      for (size_t triIndex = 0; triIndex < indicies->GetValue().dataArray.size(); ++triIndex) {
        ...
        triIndicies[0] = static_cast<size_t>(indicies->GetValue().dataArray[triIndex].dataArray[0].dataInt);
        triIndicies[1] = static_cast<size_t>(indicies->GetValue().dataArray[triIndex].dataArray[1].dataInt);
        triIndicies[2] = static_cast<size_t>(indicies->GetValue().dataArray[triIndex].dataArray[2].dataInt);
  ...
  static void LoadModelOBJ(FileString& fileName, Model& model) {
    ...
    for (size_t triIndex = 0; triIndex < indexSize; ++triIndex) {
      std::array<size_t, 3> triIndicies;
      triIndicies[0] = static_cast<size_t>(faceList[triIndex].triangleFace[0].vertexIndex);
      triIndicies[1] = static_cast<size_t>(faceList[triIndex].triangleFace[1].vertexIndex);
      triIndicies[2] = static_cast<size_t>(faceList[triIndex].triangleFace[2].vertexIndex);
      mesh.SetTriangle(triIndicies, triIndex);
    }
  }
